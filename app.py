...
def dashboard():
    ...
    try:
        ...
        emails = []  # fallback so for-loop won't crash

    # Now continue processing emails safely
    for e in emails:
        if not e['subject']:
            e['subject'] = "(No Subject)"
    # emails = fetch_emails(EMAIL, APP_PASSWORD)

    # Deduplication based on subject, timestamp, and sender
    existing_emails = {
    ...
    session.expunge_all()  # Detach all objects safely with data loaded
    session.close()
    ...
    # Optional: If summary is too short or empty, skip generating

    session.commit()
    session.close()
    return render_template('dashboard.html', emails=emails, query=query, filter_mode=filter_type)
